...
- 🌙 **农历支持**：原生支持中国农历
- 🌏 **多语言**：支持简繁中文和英文语言
- 📄 **笔记事件**：使用带有 frontmatter 属性的笔记作为事件来源，支持 [Bases](https://help.obsidian.md/bases) 集成实现高级筛选。[了解更多](./doc/note-events-zh.md)

### 事件来源模型

Yearly Glance 支持多种事件来源，你可以根据不同使用场景选择合适的存储方式：

| 来源 | 存储位置 | 适合场景 |
|------|----------|----------|
| 插件事件 | 插件数据 | 快速管理节日、生日和自定义事件 |
| 笔记事件 | 笔记 frontmatter | 需要独立笔记、详细正文、链接或附件的事件 |
| 日记事件 | 日记 frontmatter | 和 Daily Notes、日记工作流绑定的事件 |
| Bases 视图 | `.base` 查询结果 | 嵌入笔记内、根据条件筛选事件 |

关于 frontmatter 配置和 Bases 集成，请查看[笔记事件文档](./doc/note-events-zh.md)。

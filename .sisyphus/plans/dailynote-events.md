...
- [x] `npm run build:local` 成功退出
- [x] `npx tsc --noEmit` 无类型错误
- [x] `npm run lint` 通过
- [x] `npm test` 所有测试通过（含新增 DailyNote 测试）
- [x] DailyNote 事件在日历上正确显示
- [x] 编辑 DailyNote 事件后 frontmatter 正确更新
- [x] 设置 UI 中 DailyNote 区域正常工作
...
- [x] 1. Types & Enum Extension — EventSource.DAILYNOTE + dailyNoteEvent 类型
...
- [x] 2. DailyNoteService Core — 读取逻辑 + 单元测试 (TDD)
...
- [x] 4. Settings Type + Migration + DEFAULT_SETTINGS
...
- [x] 5. DailyNoteService Write Operations + Unit Tests (TDD)
...
- [x] 6. Settings UI — DailyNote 设置区域
...
- [x] 7. Calendar Integration — useYearlyCalendar Hook
...
- [x] 8. Calendar Control Bar — Visibility Toggle
...
- [x] 9. EventForm Adaptation — Disabled Fields + dailyNoteEvent Case
...
- [x] 10. EventManager — dailyNoteEvent Tab
...
- [x] 11. DailyNote Creation — via Plugin API
...
- [x] 12. EventTooltip + EventItem — DailyNote Source Display
...
- [x] 13. Build Verification + Full Test Suite
...
- [x] All tests pass
- [x] DailyNote events display correctly on calendar for current year
- [x] Settings UI for DailyNote source works correctly
- [x] Editing DailyNote event updates frontmatter list
- [x] Creating DailyNote event creates/updates correct file
- [x] Disabling DailyNote source hides events from calendar
- [x] i18n strings present in all 3 locales

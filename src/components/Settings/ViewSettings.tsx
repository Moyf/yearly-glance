...
export const ViewSettings: React.FC<ViewSettingsProps> = ({ plugin }) => {
	...
	const settingsTabs = [
		...
		{ label: t("setting.group.layout.name" as TranslationKeys), value: "layout" },
			...
			case "noteEvents":
				return (
					...
					>
						...
						{/* 预设类型属性名 */}
						<SettingsItem
							name={t("setting.general.basesEventPresetTypeProp.name")}
							desc={t("setting.general.basesEventPresetTypeProp.desc")}
						>
							<Input
								type="text"
								value={config.basesEventPresetTypeProp || ""}
								onChange={(value) => handleUpdateConfig({ basesEventPresetTypeProp: value })}
								placeholder={t("setting.general.basesEventPresetTypeProp.placeholder")}
							/>
						</SettingsItem>
					</SettingsBlock>
				);
